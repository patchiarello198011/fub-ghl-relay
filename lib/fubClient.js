...
/**
 * Generic authenticated GET for a full resource URL (used to fetch the
 * actual data referenced by a webhook, since most FUB webhook payloads
 * only include an ID and a link, not the full record).
 */
async function getResource(uri) {
  ...
  if (process.env.FUB_SYSTEM_NAME) headers['X-System'] = process.env.FUB_SYSTEM_NAME;
  if (process.env.FUB_SYSTEM_KEY) headers['X-System-Key'] = process.env.FUB_SYSTEM_KEY;
  const res = await axios.get(uri, { auth: { username: apiKey, password: '' }, headers });
  return res.data;
}

module.exports = { createPerson, getResource };
